...
duplicaPolMap :: Num a => [a] -> [a]
duplicaPolMap xd = polGeneralMap xd (2*)

mas1PolMap :: Num a => [a] -> [a]
mas1PolMap xd = polGeneralMap xd (1+)
...
esPar :: [Int] -> [Bool]
esPar xd = morePolGeneralMap xd even
--
--
--
--
...
generalFilter [] f = []
generalFilter (x:xd) f | f x = x : generalFilter xd f
                       | otherwise = generalFilter xd f

--b) dar una versión polimorfica de la misma.
polGeneralFilter :: [a] -> (a -> Bool) -> [a]
polGeneralFilter [] f = []
polGeneralFilter (x:xd) f | f x = x : polGeneralFilter xd f 
                          | otherwise = polGeneralFilter xd f

--c) redefinir la función "soloPares" en términos de dicha generalización.
soloParesPolGen :: [Int] -> [Int]
soloParesPolGen xd = polGeneralFilter xd even
--
--
--
--                 
...
generalFold [] f = 0
generalFold (x:xd) f = f x (generalFold xd f) 

--b) dar una versión polimorfica de la misma.
polGeneralFold :: [a] -> (a -> a -> a) -> a -> a
polGeneralFold [] f base = base
polGeneralFold (x:xd) f base = f x (polGeneralFold xd f base)

--c) redefinir la función "sumatoria" en términos de dicha generalización.
sumatoriaPolGen :: Num a => [a] -> a
sumatoriaPolGen xd = polGeneralFold xd (+) 0
--
--
--
--                 
...
evaluar (Division exp1 exp2) | (evaluar exp2) == 0 = error "no se puede dividir por cero"
                             | otherwise = div (evaluar exp1) (evaluar exp2)
--
--
--
--
...
binTreeGeneralFold :: a -> (a -> b -> a -> a) -> BinTree b -> a
binTreeGeneralFold casoHoja _ Hoja = casoHoja
binTreeGeneralFold casoHoja casoRama (Rama izq valor der) = casoRama 
                                                  (binTreeGeneralFold casoHoja casoRama izq)
                                                  valor
                                                  (binTreeGeneralFold casoHoja casoRama der)

profundidadGen :: BinTree a -> Int
profundidadGen = binTreeGeneralFold 0 combinaProfundidad

-- Funcion auxiliar
combinaProfundidad :: Int -> a -> Int -> Int
combinaProfundidad izq _ der = 1 + (max izq der)

-- arb3 = (Rama (Rama (Hoja) 2 (Rama (Hoja) 3 (Hoja))) 1 (Hoja))
-- arb5 = (Rama (Rama (Hoja) 2 (Rama (Hoja) 3 (Hoja))) 1 (Rama (Hoja) 4 (Rama (Hoja) 5 (Rama (Rama (Hoja) 7 (Hoja)) 6 (Hoja)))))
...
-- Recorre el arbol en pre-order.
elementosBinTree :: BinTree a -> [a]
elementosBinTree Hoja = []
elementosBinTree (Rama izq v der) = [v] ++ (elementosBinTree izq) ++ (elementosBinTree der)

elementosBinTreeFold :: BinTree a -> [a]
elementosBinTreeFold = binTreeGeneralFold [] combinarElementos

combinarElementos :: [a] -> a -> [a] -> [a]
combinarElementos izq valor der = [valor] ++ izq ++ der
